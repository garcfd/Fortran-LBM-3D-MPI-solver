C     LBM method developed from 2D Python code 
C     from Jonas Latt at Geneva Univesity (coursera) 
C
C     MPI method developed from 2D heat equation notes
C     https://warwick.ac.uk/research/rtp/sc/rse/training/advancedmpi/02_case_study_1.pdf
C     Chris Brady & Heather Ratcliffe (warwick uimversity)
C
...
C     run:     mpirun -np 16 ./warwick
      ...
      INTEGER, PARAMETER :: im = 500, jm = 100,  km = 160
...
C     Send left most strip of cells left and receive into right guard cells
      CALL MPI_Sendrecv(
     & array(1,      0:jm_l+1, 0:km_l+1,1:nvec),(jm_l+2)*(km_l+2)*nvec,
     & MPI_REAL,xmin_rank,tag,
     & array(im_l+1, 0:jm_l+1, 0:km_l+1,1:nvec),(jm_l+2)*(km_l+2)*nvec,
     & MPI_REAL,xmax_rank,tag, cart_comm, MPI_STATUS_IGNORE, ierr)

C     Send right most strip of cells right and receive into left guard cells
      CALL MPI_Sendrecv(
     & array(im_l,   0:jm_l+1, 0:km_l+1,1:nvec),(jm_l+2)*(km_l+2)*nvec,
     & MPI_REAL,xmax_rank,tag,
     & array(0,      0:jm_l+1, 0:km_l+1,1:nvec),(jm_l+2)*(km_l+2)*nvec,
     & MPI_REAL,xmin_rank,tag, cart_comm, MPI_STATUS_IGNORE, ierr)

C     Send lower most strip of cells down and receive into upper guard cells
      CALL MPI_Sendrecv(
     & array(0:im_l+1, 1,      0:km_l+1,1:nvec),(im_l+2)*(km_l+2)*nvec,
     & MPI_REAL,ymin_rank,tag, 
     & array(0:im_l+1, jm_l+1, 0:km_l+1,1:nvec),(im_l+2)*(km_l+2)*nvec,
     & MPI_REAL,ymax_rank,tag, cart_comm, MPI_STATUS_IGNORE, ierr)

C     Send upper most strip of cells up and receive into lower guard cells
      CALL MPI_Sendrecv(
     & array(0:im_l+1, jm_l,   0:km_l+1,1:nvec),(im_l+2)*(km_l+2)*nvec,
     & MPI_REAL,ymax_rank,tag, 
     & array(0:im_l+1, 0,      0:km_l+1,1:nvec),(im_l+2)*(km_l+2)*nvec,
     & MPI_REAL,ymin_rank,tag, cart_comm, MPI_STATUS_IGNORE, ierr)

C     Send back most strip of cells back and receive into front guard cells
      CALL MPI_Sendrecv(
     & array(0:im_l+1, 0:jm_l+1, 1, 1:nvec),(im_l+2)*(jm_l+2)*nvec,
     & MPI_REAL,zmin_rank,tag, 
     & array(0:im_l+1,0:jm_l+1, km_l+1,1:nvec),(im_l+2)*(jm_l+2)*nvec,
     & MPI_REAL,zmax_rank,tag, cart_comm, MPI_STATUS_IGNORE, ierr)

C     Send front most strip of cells fwd and receive into back guard cells
      CALL MPI_Sendrecv(
     & array(0:im_l+1, 0:jm_l+1, km_l,  1:nvec),(im_l+2)*(jm_l+2)*nvec,
     & MPI_REAL,zmax_rank,tag, 
     & array(0:im_l+1, 0:jm_l+1, 0,1:nvec),(im_l+2)*(jm_l+2)*nvec,
     & MPI_REAL,zmin_rank,tag, cart_comm, MPI_STATUS_IGNORE, ierr)
...
C-----set which boundaries are periodic
C     periods = [.FALSE.,.TRUE.,.TRUE.]
      periods = .FALSE.
     ...
     &     cart_comm,ierr)
      ...
      CALL MPI_Cart_shift(cart_comm,1,1,ymin_rank,ymax_rank,ierr)
      CALL MPI_Cart_shift(cart_comm,2,1,zmin_rank,zmax_rank,ierr)
...
C-----start binary VTK file
      SUBROUTINE write_binary_vtk(iter)
      USE allocated
      ...
      write(unit=string, fmt='(I6.6)') iter 
      outfile = 'genwar'//string//'.vtk'
...
C-----finish binary VTK file
      ...
      integer nits,nsav,nout

      character(10) ni,nj,nk

      real lref,re,uLB,nuLB,omega,cu,usqr,sum2,sum3
      real cr,cx,cy,cz,dx,dy,dz,dist
...
C-----obstacle
      cr = real(jm)/5.0  ! sphere radius
      cx = real(im)/4.0  ! x-loc
      cy = real(jm)/2.0  !
      cz = real(km)/2.0  !
...
C=====read obstacle from lcd array
      IF (.TRUE.) THEN
        if (rank.eq.0) write(6,*)'read obstacle from file'
        open(unit=20, file="obstacle.dat")
        read(20,*) ni,nj,nk
        if (rank.eq.0) write(6,*) ni,nj,nk
        DO i = 1, im
        DO j = 1, jm
        DO k = 1, km
          read(20,*) obs(i,j,k)
...
C      write(6,*)rank,imin_l,imax_l,jmin_l,jmax_l,kmin_l,kmax_l
      write(6,*)"rank ",rank,ymin_rank,ymax_rank,zmin_rank,zmax_rank
...
C=======streaming step
        ...
        do n = 1, nvec
           ...
           if ((nexti.ge.1).and.(nexti.le.im_l).and.
     &         (nextj.ge.1).and.(nextj.le.jm_l).and.
     &         (nextk.ge.1).and.(nextk.le.km_l)) then
             if (fout_l(i,j,k,n).ne.0.0) then
        ...
        IF (MOD(iter,nout).eq.0) THEN
            CALL gather_to_zero
            IF (rank.eq.0) write(6,*) "iter = ",iter,
     &                 "vel = ",vel(im/2,jm/2,km/2,1)
        END IF

        IF (MOD(iter,nsav).eq.0) THEN
            CALL gather_to_zero
            IF (rank.eq.0) CALL write_binary_vtk(iter)
        END IF
...
C===========
      ...
      IF (rank.eq.0) write(6,*) "solver finished"
