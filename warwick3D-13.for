...
C     run:     mpirun -np 16 ./warwick
      ...
      INTEGER, PARAMETER :: im = 200, jm = 100,  km = 100
...
C     Send left most strip of cells left and receive into left guard cells
      CALL MPI_Sendrecv(
...
C     Send right most strip of cells right and receive into right guard cells
      CALL MPI_Sendrecv(
...
C     Send lower most strip of cells down and receive into lower guard cells
      CALL MPI_Sendrecv(
...
C     Send upper most strip of cells up and receive into upper guard cells
      CALL MPI_Sendrecv(
...
C     Send back most strip of cells back and receive into back guard cells
      CALL MPI_Sendrecv(
...
C     Send front most strip of cells fwd and receive into front guard cells
      CALL MPI_Sendrecv(
...
C=======streaming at xmin/xmax block boundaries
        ...
        do n = 1, nvec
          ...
          if (imin_l.ne.1) then ! block inlet
            i = 0
            nexti = i + vec(n,1)
            nextj = j + vec(n,2)
            nextk = k + vec(n,3)
          ...
          if (imax_l.ne.im) then ! block outlet
            i = im_l+1
            nexti = i + vec(n,1)
            nextj = j + vec(n,2)
            nextk = k + vec(n,3)
...
C=======streaming at ymin/ymax boundaries
        do k = 1, km_l
        do i = 0, im_l+1
        do n = 1, nvec

          if (jmin_l.ne.1) then ! block inlet
            j = 0
            nexti = i + vec(n,1)
            nextj = j + vec(n,2)
            nextk = k + vec(n,3)
            if  (nextj.eq.1) then
            if ((nexti.ge.1).and.(nexti.le.im_l)) then
              fin_l(nexti,nextj,nextk,n) = fout_l(i,j,k,n)
            endif
            endif
          endif

          if (jmax_l.ne.jm) then ! block outlet
            j = jm_l+1
            nexti = i + vec(n,1)
            nextj = j + vec(n,2)
            nextk = k + vec(n,3)
            if (nextj.eq.jm_l) then
            if ((nexti.ge.1).and.(nexti.le.im_l)) then
              fin_l(nexti,nextj,nextk,n) = fout_l(i,j,k,n)
            endif
            endif
...
C=======streaming at kmin/kmax boundaries
        do j = 1, jm_l
        do i = 0, im_l+1
        do n = 1, nvec

          if (kmin_l.ne.1) then ! block inlet
            k = 0
            nexti = i + vec(n,1)
            nextj = j + vec(n,2)
            nextk = k + vec(n,3)
          ...
          if (kmax_l.ne.km) then ! block outlet
            k = km_l+1
            nexti = i + vec(n,1)
            nextj = j + vec(n,2)
            nextk = k + vec(n,3)
            if (nextk.eq.km_l) then
            if ((nexti.ge.1).and.(nexti.le.im_l)) then
              fin_l(nexti,nextj,nextk,n) = fout_l(i,j,k,n)
            endif
            endif
